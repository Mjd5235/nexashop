...
const Orders = () => {
    ...
    const [data1, setData1] = useState<OrderData[]>([])
    const [filterStatus, setFilterStatus] = useState<string>('All Statuses');
    const [ordersCounter, setOrdersCounter] = useState<number | null>(null)
    const [pendingCounter, setPendingCounter] = useState<number | null>(null)
    const [completedCounter, setCompletedCounter] = useState<number | null>(null)
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            setData1(filterStatus !== "All Statuses" ? (data as OrderData[]).filter(item => (item as OrderData).status === filterStatus) : data as OrderData[])
            setOrdersCounter((data as OrderData[]).length)
            setPendingCounter((data as OrderData[]).filter(item => item.status !== "Delivered").length)
            setCompletedCounter((data as OrderData[]).filter(item => item.status === "Delivered").length)
            setLoading(false)
        }
        getData()
    }, [filterStatus])
    ...
    const handleSearch = async (value: string) => {
        const { data } = await supabase.from("orders").select("*").order("created_at", { ascending: false })
        if (value === "") {
            setData1(data as OrderData[])
        } else {
            setData1((data as OrderData[]).filter(item => item.id.toLowerCase().includes(value.toLowerCase())))
        }
    }
    ...
    const handleStatusUpdate = async (value: string, ID: string) => {
        ...
        if (error) { console.error(error); toast.error("Failed to change the order status."), { id: "fchange" } } else {
            ...
            setData1(filterStatus !== "All Statuses" ? (orders as OrderData[]).filter(item => item.status === filterStatus) : (orders as OrderData[]))
            setOrdersCounter((orders as OrderData[]).length)
            setPendingCounter((orders as OrderData[]).filter(item => (item as OrderData).status !== "Delivered").length)
            setCompletedCounter((orders as OrderData[]).filter(item => (item as OrderData).status === "Delivered").length)
        }
    }
                                        ...
                                        <div className={styles.card}>
                                            ...
                                            {data1 && data1.length > 0 ? data1.map((item, index) => (
                                                <div key={item.id} className={styles.orderRow}>
                                                    ...
                                                    <div>
                                                        <div className={styles.avatar}>{(item as OrderData).user_avatar}</div>
                                                        <span className={styles.customerEmail}>{(item as OrderData).user_email}</span>
                                                    </div>
                                                    <div className={styles.colDate}>{new Date((item as OrderData).created_at).toLocaleDateString("US-en", { day: 'numeric', month: 'short', year: 'numeric' })}</div>
                                                    <div>
                                                        <span className={`${styles.statusBadge} ${(item as OrderData).status.toUpperCase() === 'DELIVERED' ? styles.badgeConfirmed : (item as OrderData).status.toUpperCase() === 'CONFIRMED' ? styles.badgeShipped : styles.statusProcessing}`}>{(item as OrderData).status}</span>
                                                    </div>
                                                    <div>
                                                        <CustomSelect
                                                            options={orderStatuses}
                                                            defaultValue={(item as OrderData).status}
